...
export default function Home() {
  ...
  return (
    <div className="min-h-screen w-full relative bg-black overflow-x-hidden">
      ...
      {/* Desktop Header */}
      <header
        className={`fixed top-4 z-[99999] hidden w-full md:flex items-center justify-between backdrop-blur-sm neon-border shadow-lg transition-all duration-300 ${
      ...
      <header className="fixed top-4 z-[99999] left-4 right-4 flex items-center justify-between md:hidden px-4 py-3 bg-background/80 backdrop-blur-sm neon-border shadow-lg rounded-full">
        ...
        {/* Contact Us - Mobile top right */}
        <button
          onClick={() => scrollToSection("contact")}
          className="rounded-md font-bold bg-gradient-to-r from-primary to-accent text-black neon-glow px-3 py-1 text-sm"
        >
          Contact Us
        </button>
        ...
        <button
          ...
          className="flex items-center justify-center w-10 h-10 rounded-full bg-background/50 neon-border ml-2"
        >
            ...
            <nav className="flex flex-col space-y-4">
              {navItems.map(item => (
                <button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className="px-4 py-3 text-left text-lg font-medium text-muted-foreground hover:text-primary rounded-lg hover:bg-background/50"
                >
                  {item.name}
                </button>
              ))}
